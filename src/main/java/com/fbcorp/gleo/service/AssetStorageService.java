...
public class AssetStorageService {
    ...
    private final Path vendorHeroDir;
    ...
    public AssetStorageService(@Value("${gleo.uploads.root:uploads}") String rootDirectory) {
        try {
            ...
            this.vendorHeroDir = rootDir.resolve("vendor_hero");
            Files.createDirectories(this.vendorDir);
            Files.createDirectories(this.menuItemDir);
            Files.createDirectories(this.vendorHeroDir);
    ...
    public String storeVendorHeroImage(MultipartFile file) throws IOException {
        if (file == null || file.isEmpty()) {
            throw new IllegalArgumentException("File is empty");
        }
        String originalFilename = StringUtils.cleanPath(
                Objects.requireNonNullElse(file.getOriginalFilename(), "vendor-hero"));
        String extension = "";
        int dotIndex = originalFilename.lastIndexOf('.');
        if (dotIndex >= 0 && dotIndex < originalFilename.length() - 1) {
            extension = originalFilename.substring(dotIndex).toLowerCase(Locale.ROOT);
        ...
        String filename = UUID.randomUUID().toString().replace("-", "") + extension;
        Path target = vendorHeroDir.resolve(filename);
        try (InputStream input = file.getInputStream()) {
            Files.copy(input, target, StandardCopyOption.REPLACE_EXISTING);
        }
        return "/uploads/vendor_hero/" + filename;
    }
